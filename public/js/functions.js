...
function initFormValidation() {
    ...
    $("#submit,#submit2").click(function () {
        ...
        var data =
            ...
            email.val() +
            "&comment=" +
            encodeURIComponent(comment.val());
...
(function ($) {
                ...
                if (attr != "#") {
                    ...
                    self.ajaxLoad(attr).done(function (data) {
                        ...
                        if (self.options.appendToBody)
                            content.appendTo(jQuery(self.options.holderBox));
                        else content.appendTo(self.ajaxHold);

                        if (btnAjax.length)
                            self.button.attr("href", btnAjax.attr("href"));
                        else self.button.hide();

                        content
                            .stop()
                            .animate({ opacity: 1 }, self.options.animSpeed);
    ...
    $.fn.ajaxPopup = function (options) {
        return this.each(function () {
            $(this).data(
                "AjaxPopup",
                new AjaxPopup($.extend(options, { holder: this }))
            );
        });
    };
})(jQuery);
